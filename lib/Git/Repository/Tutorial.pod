...
C<-b> and C<fb686b>. In the second case, it receives two arguments:
C<checkout> and C<-bfb686b>, and then B<git> recognizes the beginning
of the I<-b> option and splits C<fb686b> out of the second argument.

So, in a call such as:

    $command = $repo->run( checkout => "-b$sha1_prefix", { quiet => 0 } );

If C<$sha1_prefix> contains an initial space character, the call will be
equivalent the following shell command:

	shell> git checkout -b\ fb686b

and B<git> will receive two arguments: C<checkout> and C<-b fb686b>, from
which it will split out C< fb686b> (note the initial space).

The space after I<-b> must be removed, as otherwise the code attempts to
create a branch called C< fb686b>, which git rejects.
